...
class Qwen3VLVisualAuxPretrainLoader(Qwen3VLLoader):
    """Single-model visual aux decoder pretrain (Stage 0.5).

    Loads one Qwen3-VL model, registers visual tokens on the tokenizer,
    and patches the forward to compute future-image-token prediction loss
    directly using the model's own ViT (no separate main model).
    """

    def get_model(self, model_dir: str, config, processor, model_kwargs) -> PreTrainedModel:
        model = super().get_model(model_dir, config, processor, model_kwargs)

        from swift.model.models.latent_cot import patch_model_for_visual_aux_pretrain
        patch_model_for_visual_aux_pretrain(model, processor)
        return model
